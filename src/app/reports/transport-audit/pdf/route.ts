...
function drawFooter({
  ...
  page.drawText("MoverCare · Auditoria dos transportes", {
    x: MARGIN,
    y: 12,
    size: 8,
    font,
...
function drawTableHeader({
  ...
  const headers = [
    { label: "Data", x: 46 },
    { label: "Responsável", x: 125 },
    { label: "Ação", x: 270 },
    { label: "Status", x: 395 },
    { label: "Descrição", x: 500 },
    { label: "Transporte", x: 735 }
  ];
...
export async function GET(request: NextRequest) {
  ...
  page.drawText("Relatório de Auditoria dos Transportes", {
    x: MARGIN,
    y: PAGE_HEIGHT - 72,
    size: 23,
    font: boldFont,
  ...
  const filterText = `Filtros: ação = ${
    ...
    selectedStatus === "all" ? "Todos os status" : getStatusLabel(selectedStatus)
  } · responsável = ${actorSearch || "Todos"} · período = ${getPeriodLabel(
    selectedPeriod
  )}`;
  ...
  const cards = [
    { title: "REGISTROS", value: logs.length, description: "Eventos filtrados" },
    { title: "CRIADOS", value: created, description: "Transportes criados" },
    { title: "ACEITOS", value: accepted, description: "Aceites registrados" },
    { title: "INICIADOS", value: started, description: "Inícios registrados" },
    { title: "CONCLUÍDOS", value: completed, description: "Finalizações" },
    { title: "CRÍTICOS", value: critical, description: "Cancelados/falhas" }
  ];
  ...
  page.drawText("Histórico de eventos dos transportes", {
    x: MARGIN,
    y: cardTop - 34,
    size: 15,
    font: boldFont,
  ...
  for (const log of logs) {
    ...
    const transportLines = wrapText({
      text: log.transport_id,
      maxWidth: 70,
      font: regularFont,
      fontSize: 7
    }).slice(0, 4);
    ...
    statusLines.forEach((line, index) => {
      page.drawText(line, {
        ...
        y: y - 10 - index * 10,
        size: 7.5,
        font: index === 1 ? boldFont : regularFont,
        color: index === 1 ? rgb(0.05, 0.09, 0.16) : rgb(0.39, 0.45, 0.55)
      });
    });
